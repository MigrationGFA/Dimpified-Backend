...
const userHelpCenter = async (req, res) => {
  ...
  try {
    const { userId, reason, message, ecosystemDomain } = req.body;
    const details = ["userId", "reason", "message", "ecosystemDomain"];
...
const sendFeedback = async (req, res) => {
  try {
    ...
    if (!requestId || !subject || !message) {
      return res
        .status(400)
        .send({ error: "requestId, subject, and message are required" });
    }
    ...
    res
      .status(200)
      .send({ message: "Help request feedback email sent successfully" });
  } catch (error) {
...
const creatorhelpCenterDashBoard = async (req, res) => {
  try {
    const { creatorId } = req.params;
    const totalHelpRequests = await HelpCenter.count({
      where: { creatorId: creatorId },
    });

    const totalCompletedHelpRequest = await HelpCenter.count({
      where: {
        creatorId: creatorId,
        status: "completed",
      },
    ...
    const totalPendingHelpRequest = await HelpCenter.count({
      where: {
        creatorId: creatorId,
        status: "pending",
      },
